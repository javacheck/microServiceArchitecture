...
public class Main {
	...
	public static void main(String[] args) {
		...
		try {
			...
			RefundReqData refundReqData = new RefundReqData(null, "20000173",
					null, "10001", 1, 1, Configure.getMchid(),
					"CNY");

			WXPay.doRefundBusiness(refundReqData, new RefundBusiness.ResultListener() {
				
				@Override
				public void onRefundSuccess(RefundResData refundResData) {
					System.out.println("onRefundSuccess=" + refundResData);
				}
				
				@Override
				public void onRefundFail(RefundResData refundResData) {
					System.out.println("onRefundFail=" + refundResData);
				}
				
				@Override
				public void onFailBySignInvalid(RefundResData refundResData) {
					System.out.println("onFailBySignInvalid=" + refundResData);
				}
				
				@Override
				public void onFailByReturnCodeFail(RefundResData refundResData) {
					System.out.println("onFailByReturnCodeFail=" + refundResData.getErr_code());
				}
				
				@Override
				public void onFailByReturnCodeError(RefundResData refundResData) {
					System.out.println("onFailByReturnCodeError=" + refundResData);
				}
			});
			// --------------------------------------------------------------------
