...
import cn.lastmiles.pay.pos.wxpay.service.IServiceRequest;
...
public class HttpsRequest implements IServiceRequest{
    ...
    public HttpsRequest(Configure configure) throws UnrecoverableKeyException, KeyManagementException, NoSuchAlgorithmException, KeyStoreException, IOException {
        init(configure);
    }
    ...
    private void init(Configure configure) throws IOException, KeyStoreException, UnrecoverableKeyException, NoSuchAlgorithmException, KeyManagementException {
        ...
        byte[] arr = Base64.decode(configure.getCertIo());
        ByteArrayInputStream input = new ByteArrayInputStream(arr);
        try {
            keyStore.load(input, configure.getCertPassword().toCharArray());//设置证书密码
