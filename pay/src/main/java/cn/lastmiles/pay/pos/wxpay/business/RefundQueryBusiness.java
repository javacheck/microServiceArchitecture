package cn.lastmiles.pay.pos.wxpay.business;

import cn.lastmiles.pay.pos.wxpay.common.*;
import cn.lastmiles.pay.pos.wxpay.protocol.refund_query_protocol.RefundOrderData;
import cn.lastmiles.pay.pos.wxpay.protocol.refund_query_protocol.RefundQueryReqData;
import cn.lastmiles.pay.pos.wxpay.protocol.refund_query_protocol.RefundQueryResData;
import cn.lastmiles.pay.pos.wxpay.service.RefundQueryService;
...
public class RefundQueryBusiness {
    ...
    public RefundQueryBusiness(Configure configure) throws IllegalAccessException, ClassNotFoundException, InstantiationException {
    	setConfigure(configure);
    	refundQueryService = new RefundQueryService(configure);
    }
    ...
    public void run(RefundQueryReqData refundQueryReqData,RefundQueryBusiness.ResultListener resultListener) throws Exception {
        ...
        //--------------------------------------------------------------------
    	refundQueryReqData.setAppid(configure.getAppID());//微信分配的公众号ID（开通公众号之后可以获取到）
    	refundQueryReqData.setMch_id(configure.getMchID());//微信支付分配的商户号ID（开通公众号的微信支付功能之后可以获取到）
    	refundQueryReqData.setSign(Signature.getSign(refundQueryReqData.toMap(),configure.getKey()));//把签名数据设置到Sign这个属性中
		...
		*/
        ...
        if (refundQueryResData.getReturn_code().equals("FAIL")) {
            ...
            if (!Signature.checkIsSignValidFromResponseString(refundQueryServiceResponseString,configure.getKey())) {
