...
import cn.lastmiles.pay.pos.wxpay.protocol.refund_protocol.RefundResData;
import cn.lastmiles.pay.pos.wxpay.service.RefundService;
...
public class RefundBusiness {
    ...
    public RefundBusiness(Configure configure) throws IllegalAccessException, ClassNotFoundException, InstantiationException {
    	this.configure=configure;
    	refundService = new RefundService(configure);
    }
    ...
    public void run(RefundReqData refundReqData,ResultListener resultListener) throws Exception {
        ...
        if (refundResData.getReturn_code().equals("FAIL")) {
            ...
            if (!Signature.checkIsSignValidFromResponseString(refundServiceResponseString,configure.getKey())) {
