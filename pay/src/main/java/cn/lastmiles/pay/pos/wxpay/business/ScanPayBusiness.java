...
import cn.lastmiles.pay.pos.wxpay.common.Log;
...
import cn.lastmiles.pay.pos.wxpay.protocol.reverse_protocol.ReverseResData;
import cn.lastmiles.pay.pos.wxpay.service.ReverseService;
import cn.lastmiles.pay.pos.wxpay.service.ScanPayQueryService;
import cn.lastmiles.pay.pos.wxpay.service.ScanPayService;
...
public class ScanPayBusiness {
    ...
    public ScanPayBusiness(Configure configure) throws IllegalAccessException, ClassNotFoundException, InstantiationException {
        this.configure=configure;
    	scanPayService = new ScanPayService(configure);
    ...
    //循环调用订单查询API的次数
    private int payQueryLoopInvokedCount = 10;
    ...
    public void run(ScanPayReqData scanPayReqData, ResultListener resultListener) throws Exception {
    	...
    	final String outTradeNo = scanPayReqData.getOut_trade_no();
        ...
        if (scanPayResData.getReturn_code().equals("FAIL")) {
            ...
            //--------------------------------------------------------------------
            if (!Signature.checkIsSignValidFromResponseString(payServiceResponseString,configure.getKey())) {
