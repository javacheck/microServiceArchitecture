...
// String filePath = request.getSession().getServletContext().getRealPath("/") + "resources/images/logoImages/llhlogo.png";
    ...
    public String addLogo_QRCode(BufferedImage bim, File logoPic, LogoConfig2 logoConfig, String productName)
    {
        try
        {
            /** * 读取二维码图片，并构建绘图对象 */
...
// outg.drawString(productName, 0, image.getHeight() + (outImage.getHeight() - image.getHeight())/2 + 5 ); //画文字
            ...
            ImageIO.write(image, "png", new File("S:/TDC-" + new Date().getTime() + "test.png"));
            //TODO
...
class LogoConfig2
{
    ...
    public LogoConfig2(Color borderColor, int logoPart)
    {
