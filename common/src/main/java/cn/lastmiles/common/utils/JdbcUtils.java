...
public class JdbcUtils {
	...
	private final static Configuration cfg = new Configuration(new Version(
			"2.3.23"));
	...
	public static void init(NamedParameterJdbcTemplate jt) {
		...
		// 加载sql
		URL url = ClassLoaderUtil.getResourceUrl("/lm_sql");
		if (url != null) {
				...
				protected void onEntry(EntryData entryData) throws Exception {
					...
					if (filename.endsWith(".xml")) {
						...
						byte[] bytes = StreamUtil
								.readAvailableBytes(inputStream);
						...
						if (root != null) {
							...
							for (Element e : sqlElement) {
								String id = FileNameUtil.getBaseName(filename)
										+ "." + e.getAttribute("id");
	...
	public static List<Map<String, Object>> queryForList(String sqlId,
			Map<String, Object> parameters) {
		String sql = getSql(sqlId, parameters);
		return namedParameterJdbcTemplate.queryForList(sql, parameters);
	}

	public static <T> List<T> queryForList(String sqlId,
			Map<String, Object> parameters, Class<T> cl) {
		String sql = getSql(sqlId, parameters);
	...
	public static <T> T queryForObject(String sqlId,
			Map<String, Object> parameters, Class<T> cl) {
	...
	public static int deleteById(Class<?> cl, Object id) {
		...
		p.put(idName, id);
		return namedParameterJdbcTemplate.update("delete from " + tableName
				+ " where " + idName + " = :" + idName, p);
	}
	...
	public static <T> T findById(Class<T> cl, Object id) {
		...
		return BeanUtils.toBean(cl, namedParameterJdbcTemplate.queryForMap(
	...
	public static <T> T findByObj(Class<T> cl, String objName, Object obj) {
		...
		return BeanUtils.toBean(cl, namedParameterJdbcTemplate.queryForMap(
				"select * from " + tableName + " where " + objName + " = :"
						+ objName, paramMap));
	}
			...
			if (c != null) {
				try {
					...
					if (!contain(colName, noUpdateColumn)){
						Method getMethod = ReflectionUtils.findMethod(cl, "get"
								+ StringUtils.capitalize(field.getName()));
						if( null == getMethod ){ // 首字母大写取不到数据则进行另外的处理
							getMethod = ReflectionUtils.findMethod(cl, "get"
									+ (field.getName()));
						}
						map.put(colName,
								ReflectionUtils.invokeMethod(getMethod, obj));
						if (flag) {
							sql.append(" set ");
							flag = false;
						} else {
	...
	public static void updateForAnd(String tableName,
			Map<String, Object> updateMap, Map<String, Object> whereAndMap) {
		StringBuilder sql = new StringBuilder("update ");
		sql.append(tableName);
		sql.append(" set ");
