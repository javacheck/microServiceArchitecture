...
import cn.lastmiles.alipay.model.builder.trade.AlipayTradeCancelRequestBuilder;
...
import cn.lastmiles.alipay.service.inter.AlipayTradeService;
...
public abstract class AbsAlipayTradeService extends AbsAlipayService implements AlipayTradeService {
    ...
    public AlipayF2FRefundResult tradeRefund(AlipayTradeRefundRequestBuilder builder) {
		...
		if(!Constants.VALIDATESUCCESS.equals(validate)){
			return new AlipayF2FRefundResult(validate);
		}
        ...
        request.putOtherTextParam("app_auth_token", builder.getAppAuthToken());
        request.setBizContent(builder.toJsonString());
        logger.info("trade.refund bizContent:" + request.getBizContent());
    ...
    public AlipayF2FPrecreateResult tradePrecreate(AlipayTradePrecreateRequestBuilder builder) {
    	String validate = validateBuilder(builder);
		if(!Constants.VALIDATESUCCESS.equals(validate)){
			return new AlipayF2FPrecreateResult(validate);
		}
    ...
    protected AlipayTradeCancelResponse tradeCancel(AlipayTradeCancelRequestBuilder builder) {
        ...
        request.putOtherTextParam("app_auth_token", builder.getAppAuthToken());
        request.setBizContent(builder.toJsonString());
        logger.info("trade.cancel bizContent:" + request.getBizContent());
    ...
    protected AlipayTradeCancelResponse cancelPayResult(AlipayTradeCancelRequestBuilder builder) {
        ...
        if (needRetry(response)) {
        	...
        	logger.warn("begin async cancel request:" + builder);
            asyncCancel(builder);
        }
        return response;
    }
