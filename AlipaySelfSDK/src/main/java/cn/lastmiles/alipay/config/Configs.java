package cn.lastmiles.alipay.config;
...
public class Configs {
    ...
    /**
     * 商户应用APPID，只要您的应用中包含店铺接口且是开通状态，就可以用此应用对应的appid。开发者可登录开放平台-管理中心-对应应用中查看
     */
    ...
    public synchronized static void init(String filePath) {
        ...
        String readCharset = configs.getString("charset");
    	if( null != readCharset && !"".equals(readCharset)){
    		charset = readCharset;
    	}
    	
    	String readFormat = configs.getString("format");
    	if( null != readFormat && !"".equals(readFormat)){
    		format = readFormat;
    	}
