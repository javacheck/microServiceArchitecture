...
public class PostSender implements Sender {
	...
	public UFileResponse send(UFileClient ufileClient, UFileRequest request) {
		String uri = "http://" + request.getBucketName() + ufileClient.getProxySuffix() + "/";	
		...
		try {
			...
			ContentType ct = ContentType.create(request.getContentType());
			FileBody fileBody = new FileBody(file, ct, file.getName());
			
			HttpEntity entity = MultipartEntityBuilder.create()
					.setMode(HttpMultipartMode.BROWSER_COMPATIBLE)
					.addPart("Authorization", new StringBody(request.getAuthorization(), ct))
					.addPart("FileName", new StringBody(request.getKey(), ct))
