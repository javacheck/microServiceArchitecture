...
<title>${empty productBrand ? '添加' : '修改' }商品品牌</title> 
<script type="text/javascript">

$(function(){
	
	var type="${empty productBrand ? 0:1}";
	...
	//绑定 提交方法
	$("#productBrandAddBtn").click(function() {
		var id=$("#id").val();
		...
		$("input[name='brandName']").each(function(index,item){
			...
			lm.postSync("${contextPath}/productBrand/list/ajax/exist",{id:id,name:$.trim($(this).val()),storeId:storeId},function(data){
				if(data==1){
					lm.alert("商品品牌名称<span style='color:red'>"+nameVal+"</span>已存在！");
					flag=false;
					return false;
				}
			});
		...
		if(isRepeat(arrName)==true){
			lm.alert("商品品牌有重复，请检查！");
			return;
		}
		if(flag==false){
	 		return;
	 	}
		$("#productBrandAddBtn").prop("disabled","disabled");
		$("#brandSave").submit();
...
function nameAdd(){
	...
	$("#nameAddDiv").append('<div class="form-group " id="nameAdd'+addflag+'" >'+
									'<label class="col-md-1 control-label"><span style="color: red;font-size: 15px">*</span>品牌名称</label>'+
									'<div class="col-md-2">'+
										'<input type="text"  name="brandName" value="" class="form-control"  maxlength="120"/>'+
									'</div>'+
...
<body>
	<div class='panel'>
		<div class='panel-heading'>
			<strong>
			<i class='icon-plust'></i>${empty productBrand ? '添加' : '修改' }商品品牌
			</strong>
		</div>
		<div class='panel-body'>
			<form method='post'  id="brandSave" action="${contextPath}/productBrand/save" enctype="multipart/form-data">
				<div  class='form-horizontal'>
					<input id="id" name="id" type="hidden" value="${productBrand.id }" />
					   <c:if test="${isSys==true }">  
					   		<c:if test="${empty productBrand }">
							...
							<c:if test="${not empty productBrand }">
								<div class="form-group" >
									<label class="col-md-1 control-label"><span style="color: red;font-size: 15px">*</span>商家</label>
									<div class="col-md-2">
										<input name="" id="" readonly="readonly" value="${productBrand.storeName }" class="form-control" />
									</div>
								</div>
							</c:if>
					   </c:if>  
					<c:if test="${empty productBrand }">
						<div id="nameAddDiv">
							<div class="form-group " id="nameAdd0">
								...
								<input type="button"  id="nameAddBtn" class="btn btn-small btn-warning"  value="添加品牌名称" onclick="nameAdd();" />
							</div>
						</div>
					</c:if>
					<c:if test="${not empty productBrand }">
						<div class="form-group">
							<label class="col-md-1 control-label"><span style="color: red;font-size: 15px">*</span>品牌名称</label>
							<div class="col-md-2">
								<input type='text' name="brandName" value="${productBrand.name }" class='form-control'   maxlength="120"/>
							</div>
						</div>
					</c:if>
					...
					<div class="form-group">
						<div class="col-md-offset-1 col-md-1 0" style="float:left">
							<input type="button"  id='productBrandAddBtn' class='btn btn-primary' value="${empty productBrand ? '添加' : '修改' }" />
						</div>
					</div>
				</div>
			</form>
