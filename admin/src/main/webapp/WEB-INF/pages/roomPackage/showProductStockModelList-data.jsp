	...
	if(cacheArray.length > 0){
		$("#show_ProductStock_Model input[type='checkbox']").each(function(key,value){
			for (var i = 0; i < cacheArray.length; i++) {
				if(cacheArray[i] == ($(value).attr("stockIdSign")) ){
		...
		<m:table pageNo="${page.pageNo }" pages="${page.pages }" pageSize="${page.pageSize }" total="${page.total }">
			...
			<tbody>
				<c:forEach items="${page.data}" var="productStock">
					<tr style="cursor: pointer;" id="${productStock.id }" stockIdSign="${productStock.id }" onclick="trSelectEvent(this);">
						<td><input type="checkbox" id="${productStock.id }" stockIdSign="${productStock.id }" onclick="checkboxSelectEvent(this);" /></td>
						<td>${productStock.id }</td>
						<td>${productStock.storeName }</td>
						<td>${productStock.store.mobile }</td>
						<td>${productStock.productName } </td>
						<td><c:if test="${productStock.attributeValuesListJointValue != null }"> ${productStock.attributeValuesListJointValue }</c:if> </td>
