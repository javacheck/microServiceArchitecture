$(function(){
	$.loadData = function(formId,successmsg,callback,beforeSearch){
		if (beforeSearch()){
		var $listForm = $("#" + formId);
		...
		$.ajax({
			...
			success : function(data,textStatus,xhr) {
				if(xhr.getResponseHeader("sessionstatus") == "0"){
					self.parent.location.href=xhr.getResponseHeader("loginUrl");
				...
				$pager.pager({
					...
					buttonClickCallback : function(id){
						$pageNo.val(id);
						$.loadData(formId,successmsg,callback,beforeSearch);
					}
				});
				if (_total == 0){
					...
					// 取到页面数据的显示条数 -- 当数据(_total)大于0时才进行计算
					var _pageSize = $listTable.attr("pageSize");
					if( _total % _pageSize == 0 ){ 
						if( _pageNo > _pageCount ){ // 当显示的页面标识数 大于 数据的总页数，则将页面跳转到最大页数  
							$pageNo.val(_pageCount);
		...
		// 查找
		$searchButton.bind("click",function(event,pageNo){
			if (beforeSearch()){
