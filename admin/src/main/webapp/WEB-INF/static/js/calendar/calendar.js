...
(function(root, factory) {
    ...
    var defaults = {
        ...
        TEMPLATE = [
            ...
            '<a href="javascript:;" style="text-decoration:none" data-calendar-display-date class="calendar-display">',
            '<span class="m">{yyyy} 年{mm} 月</span>',
            '</a>',
    ...
    Calendar.prototype = {
        ...
        getDayItem: function(y, m, d, f) {
            ...
            if (dt.isSame(y, m, d)) {
            	...
            	currentDate = dt.format("yyyy-mm");
            	if( undefined == this.options.dataArray || null == this.options.dataArray ){
            		this.options.dataArray = this.options.getDataArray.call();             		
            	}
            }
        ...
        initFunction: function(dataArray){
        	if( null != dataArray && undefined != dataArray ){
        		for( var i=0 ; i<=dataArray.length; i++){
                 	this.selectedDay(dataArray[i],'');            	
                }
        	}
        },
        ...
        getDisDateValue: function() {
        	 ...
        	 * 获取显示的日期值(截取年月)
        	 */
        	var disDateHtml = this.$disDate.html();
        	if( undefined == disDateHtml || '' == disDateHtml ){
        		var now = new Date();
        		return [now.format("yyyy"),now.format("mm")];
        	}
        	// 截取日期,如果是用/划分,则截取数据,否则使用年做区分
        	var biasArray = disDateHtml.split('/');
        	if( biasArray.length <= 1 ){
        		biasArray =  disDateHtml.split('年');
        		
        		y = Number(biasArray[0]),
        		m = Number(biasArray[1].match(/\d{1,2}/)[0]);
        		return [y, m];
        	} else {
        		y = Number(biasArray[0]),
                m = Number(biasArray[1].match(/\d{1,2}/)[0]);
        ...
        selectedDay: function(d, type,show) {
            var arr = this.getDisDateValue(),
                 ...
                 * 选中日期的时候,切换样式 
                 */ 
            	var a = this.$dateItems.children(':eq(1)')
                .find('[' + ITEM_DAY + ']:not(.' + NEW_DAY_CLASS + ', .' + OLD_DAY_CLASS + ')');
            	if(show){
            		a.filter(function(index) {
            			return parseInt(this.innerHTML) === parseInt(d);
            		}).addClass(SELECT_CLASS);
            	} else {
            		a.filter(function(index) {
            			return parseInt(this.innerHTML) === parseInt(d);
            		}).toggleClass(SELECT_CLASS);            		
            	}
            ...
            if (type) {
                ...
                y = ret.y;
                m = ret.m;
                this.options.viewChange('date', y, m,_this);
            } else {
        ...
        event: function() {
            ...
            _this.$element.on('click',DISPLAY_VD, function() {
                ...
                vc('month', arr[0], arr[1],_this);

            });
            _this.$element.on('click',DISPLAY_VM, function() {
            ...
            _this.$element.on('click','[' + ITEM_DAY + ']', function() {
                ...
                _this.options.onSelected.call(this, 'date', day, $(this).data(MARK_DATA),_this);
                ...
                vc('date', y, m,_this);
                _this.options.onSelected.call(this, 'month', new Date(y, m - 1),_this);
            });
            ...
            _this.$element.on('mouseleave','[' + ITEM_DAY + ']', function() {
                _this.$label.hide();
            });
        },
        resize: function() {
        ...
        init: function() {
            ...
            this.initFunction(this.dataArray);
        },
